...
%it seems more as a symptom of a theory of phonology that is not 
...
%for testing the hypothesis of correlation, we set the null hypothesis.
...
%by the number of existing words, we get a monotonically decreasing line, as expected 
...
\begin{sidewaysfigure}
  ...
  \caption{Number of words in English with a certain length, measured as the number of letters, phones or syllables.}
  ...
  \caption{Frequency of occurrence of words in English with a certain length, measured as the number of letters, phones or syllables.}
...
\begin{sidewaysfigure}
  ...
  \caption{The data here presented uses the utterances duration of speech samples from the four on-line dictionaries. The durations were first normalized within each dictionary. Only the words found in all the four dictionaries were used.}
...
%some observations (some values of $k$, the rank).
...
%using the approach proposed by \cite{}, 
...
Comparing two systems with $s_1$ and $s_2$ number of symbols in inventory, and constants $K_1$ and $K_2$, 
...
We might observe that there is a significant deviation from the value estimated from the theoretical
...
\end{tabular}
\caption{The effect of the parameter $q$ on the entropy. A greater value of $q$ increases the entropy and the increase step is larger when the value of $s$ is bigger.}
...
a better approximation, we believe it is necessary to take into account 
...
by the method here proposed. We are taking into account the coherence in Zipf (see Section \ref{sec:coherencezipf})
